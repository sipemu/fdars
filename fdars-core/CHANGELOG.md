...
- **`FdMatrix::submatrix`**, **`select_rows`**, **`select_columns`**: Efficient matrix subsetting methods.
- **`RegressionLayer.selection_extra`**: Model selection details (`serde_json::Value` behind serde feature).
- **`AlignmentLayer::to_karcher_mean_result()`**: Reconstruct `KarcherMeanResult` from layer fields for elastic FPCA/changepoint.
- **`SpmChartLayer::from_chart()`**: Lossless construction from `SpmChart` with all FPCA fields.
- **`SpmChartLayer::can_monitor()`**: Check if layer has enough state for field-based monitoring.

### Improved

- **API documentation**: Added import guidance section and feature flag table to `lib.rs`.
- **`linalg` feature**: Clarified in Cargo.toml that it gates `faer`/`anofox-regression` only; basic linear algebra (Cholesky, OLS) is always available.

...
- **Phase tolerance bands** (`tolerance/elastic.rs`): `phase_tolerance_band` maps warping functions to tangent space of the Hilbert sphere via shooting vectors, computes FPCA tolerance bands, and maps bounds back to warping functions (γ). `elastic_tolerance_band_joint` computes both amplitude and phase bands in a single Karcher mean pass. New types: `PhaseToleranceBand`, `ElasticToleranceBandResult`, `ElasticToleranceConfig`
- 133 new tests: 55 SPM unit, 32 scalar-on-shape unit, 34 integration/validation, 12 phase/joint tolerance band tests
- SPM and scalar-on-shape re-exports in `lib.rs` and `prelude.rs`

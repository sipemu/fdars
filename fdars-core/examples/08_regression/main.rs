...
use fdars_core::regression::{fdata_to_pc, fdata_to_pls, FpcaResult};
...
fn main() {
    ...
    let ncomp = 4;
    if let Ok(fpca) = fdata_to_pc(&data_mat, ncomp, &t) {
    ...
    let ncomp_pls = 3;
    if let Ok(pls) = fdata_to_pls(&data_mat, &y, ncomp_pls, &t) {

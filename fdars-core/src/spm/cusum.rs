...
///
/// Construct via `CusumConfig::default()`, then assign the fields you need (e.g. `let mut c = CusumConfig::default(); c.field = …;`). This struct is `#[non_exhaustive]`, so external crates cannot build it with a struct literal — not even functional-update `..Default::default()` form.
#[non_exhaustive]

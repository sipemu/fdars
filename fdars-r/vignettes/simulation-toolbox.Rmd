...
```{r decay-comparison, fig.height = 6}
par(mfrow = c(2, 1))
...
lines(t, mean_fn(t), col = "red", lwd = 3)
legend("topright", "True Mean", col = "red", lwd = 3)
```
...
```{r multivariate-plot, fig.height = 6}
par(mfrow = c(2, 1))
...
The package also offers Gaussian Process simulation via `make_gaussian_process()`:
...
set.seed(42)
fd_gp <- make_gaussian_process(n = 10, argvals = t,
                                kernel = kernel_brownian())
...
# 3. Visualize
par(mfrow = c(1, 2))
plot(group1, main = "Group 1", col = rgb(0, 0, 1, 0.5))
lines(t, mean_group1(t), col = "blue", lwd = 3)
plot(group2, main = "Group 2", col = rgb(1, 0, 0, 0.5))
lines(t, mean_group2(t), col = "red", lwd = 3)
par(mfrow = c(1, 1))
```
...
| `make_gaussian_process()` | GP simulation with covariance kernels |

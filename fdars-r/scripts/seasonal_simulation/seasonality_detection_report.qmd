...
This report describes and compares six methods for detecting seasonality in functional time series data. We evaluate each method's performance across different scenarios including varying seasonal strengths, non-linear trends, and different trend types.
...
**Method**: Uses `select.basis.auto()` with AIC criterion.

**Note**: The internal FFT-based seasonal hint has a threshold that is too low (2.0 instead of ~6.0), causing high false positive rates. Use `use.seasonal.hint = FALSE`.
...
![Detection rates by seasonal strength](seasonality_detection_comparison.pdf){#fig-detection-rates width=100%}
...
![Precision-Recall curves](seasonality_detection_details.pdf){#fig-pr-curves width=100%}
...
![Detection rates heatmap by seasonal and trend strength](seasonality_detection_trend_heatmaps.pdf){#fig-trend-heatmap width=100%}
...
![FPR when no seasonality is present, across trend strengths](seasonality_detection_trend_pr.pdf){#fig-trend-fpr width=100%}
...
![F1 scores by trend type](seasonality_detection_trend_types_f1.pdf){#fig-trend-types-f1 width=100%}

### FPR by Trend Type

![Example trend functions](seasonality_detection_trend_types_examples.pdf){#fig-trend-examples width=100%}

![FPR across trend types and strengths](seasonality_detection_trend_types_fpr.pdf){#fig-trend-types-fpr width=100%}
...
3. **Basis Auto has a bug**: Internal FFT threshold of 2.0 is too low (should be ~6.0)
...
- `seasonal_basis_comparison.R` — AIC comparison study

PDF outputs contain the visualizations referenced in this report.

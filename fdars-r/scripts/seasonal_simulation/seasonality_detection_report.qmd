...
detect_aic <- function(fd_single) {
  result <- tryCatch({
    fourier_result <- fdata2basis(fd_single, type = "fourier", nbasis = 11)
    pspline_result <- pspline(fd_single, nknots = 10, lambda = "gcv")
    aic_fourier <- basis.aic(fourier_result)
    aic_pspline <- basis.aic(pspline_result)
    list(score = aic_pspline - aic_fourier, detected = (aic_pspline - aic_fourier) > detection_thresholds$aic)
...
detect_fft <- function(fd_single) {
  result <- tryCatch({
    period_result <- estimate.period(fd_single, method = "fft", return_confidence = TRUE)
...
detect_acf <- function(fd_single) {
  result <- tryCatch({
    period_result <- estimate.period(fd_single, method = "acf", return_confidence = TRUE)
    score <- if (!is.null(period_result$confidence)) period_result$confidence else NA
...
detect_mp <- function(fd_single) {
  result <- tryCatch({
    mp_result <- matrix.profile(fd_single, subsequence_length = 12)
...
**Concept**: Compare model fit between Fourier basis (assumes periodicity) and P-splines (flexible, non-periodic).
...
metrics_display <- metrics %>%
  arrange(desc(F1)) %>%
  mutate(across(c(Accuracy, Precision, Recall, FPR, F1), ~sprintf("%.1f%%", .x * 100))) %>%
...
trend_summary <- trend_tpr %>%
  ...
  mutate(across(where(is.numeric), ~sprintf("%.0f%%", .x * 100)))
...
rednoise_display <- rednoise_fpr %>%
  mutate(across(all_of(detection_cols), ~sprintf("%.0f%%", .x * 100))) %>%
  rename(
    `phi` = ar_coef,
...
outlier_display <- outlier_tpr %>%
  ...
  mutate(across(-config, ~sprintf("%.0f%%", .x * 100))) %>%
  rename(
...
# ROC and PR Curve Analysis {#sec-roc}
...
# Compute ROC curves from baseline results
compute_roc <- function(scores, ground_truth, method_name) {
  ...
  thresholds <- quantile(scores, probs = seq(0, 1, length.out = 100), na.rm = TRUE)

  roc_data <- lapply(unique(thresholds), function(t) {
    pred <- scores > t
  ...
  df <- do.call(rbind, roc_data)
  df$Method <- method_name
  df
}

# For this we need scores, not just detected. Use baseline strength as proxy
# This is simplified - in practice we'd store scores during simulation

# Create simplified ROC visualization based on metrics
metrics_for_roc <- metrics %>%
  filter(!is.na(Recall) & !is.na(FPR)) %>%
  select(Method, TPR = Recall, FPR)

ggplot(metrics_for_roc, aes(x = FPR, y = TPR, color = Method)) +
  geom_point(size = 4) +
  ...
  labs(
    title = "Operating Points: TPR vs FPR (All 13 Methods)",
    subtitle = "Points represent default threshold performance",
...
final_ranking <- metrics %>%
  ...
  mutate(
    Rank = row_number(),
    F1 = sprintf("%.1f%%", F1 * 100),
    FPR = sprintf("%.1f%%", FPR * 100),
    Recall = sprintf("%.1f%%", Recall * 100),
    Best_For = c(
      "Time-varying signals", "Known period", "Trend robustness", "High precision",
      "Irregular sampling", "FFT+ACF hybrid", "Decomposition", "Model comparison",
      "Subspace analysis", "Non-sinusoidal", "Trended data", "No tuning", "Conservative"
    )[1:nrow(metrics)]
  ) %>%

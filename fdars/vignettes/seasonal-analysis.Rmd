...
yearly cycles). The `estimate_period()` function returns only the dominant period.
...
The `detect_multiple_periods()` function uses an iterative residual approach implemented
...
- For **harmonics of the same fundamental**: `estimate_period()` correctly finds the
...
- `estimate_period()` - For accurate period estimation on trending data
- `detect_peaks()` - For correct peak prominence calculation
- `seasonal_strength()` - For true seasonality measurement
- `detect_multiple_periods()` - Uses "auto" by default
...
`detect_peaks` handles these cases by analyzing local signal structure.
...
Some signals have periods that drift or change over time. The `instantaneous_period()`
...
**Alternative for abrupt changes:** Use `detect_seasonality_changes()` to find regime
...
The `classify_seasonality()` function returns one of four classifications based on seasonal
...
| Function | Purpose |
|----------|---------|
| `estimate_period()` | Estimate seasonal period (FFT or ACF method) |
| `detect_multiple_periods()` | Detect multiple concurrent periodicities |
| `detect_peaks()` | Find and characterize peaks (with auto GCV smoothing) |
...
| `detect_seasonality_changes_auto()` | Auto threshold using Otsu's method |
| `instantaneous_period()` | Period estimation for smoothly drifting signals |
| `analyze_peak_timing()` | Analyze peak timing variability across cycles |
| `classify_seasonality()` | Classify seasonality type (stable/variable/intermittent) |
...
- Period unknown, data has trend: `estimate_period(detrend_method = "linear")`
- Period unknown, multiple independent periods: `detect_multiple_periods()`
- Period varies smoothly over time: `instantaneous_period()`

**Peak detection:**

- Clean data: `detect_peaks()` with default parameters
...
- Single measurement: `seasonal_strength()`
- Data with trend: `seasonal_strength(detrend_method = "linear")`
- Track changes over time: `seasonal_strength_curve()`
- Detect when seasonality stops: `detect_seasonality_changes_auto()`

**Short series (3-5 cycles):**

- Characterize timing shifts: `analyze_peak_timing()`
- Classify pattern type: `classify_seasonality()`

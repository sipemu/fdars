...
For short series like yearly data with only 3-5 complete cycles, fdars provides
specialized functions to analyze peak timing variability and classify seasonality.
...
| `detect_seasonality_changes_auto()` | Auto threshold (Otsu's method) |
...
- **Period unknown, noisy**: Use `detect_peaks()` with `smooth_first = TRUE`
...
- **Short series (3-5 years)**: Use `analyze_peak_timing()` and `classify_seasonality()`
- **Unknown threshold**: Use `detect_seasonality_changes_auto()` with Otsu's method

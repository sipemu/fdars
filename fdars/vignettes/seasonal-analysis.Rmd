...
Before detecting peaks, let's see what P-spline smoothing does to noisy data:
...
Smoothing removes high-frequency noise while preserving the underlying seasonal pattern.
Peak detection on the smoothed data will find the true peaks, not noise spikes.
...
**Key insight:** For real data, always use `smooth_first = TRUE` with `smooth_lambda = NULL`
to let GCV automatically select the smoothing parameter.
...
- Period unknown, multiple independent periods: Use iterative residual approach

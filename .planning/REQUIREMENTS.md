...
- [x] **PERF-06**: A conditional nalgebra-vs-faer SVD comparison at fdars' real problem sizes, performed only if benchmarks show SVD to be a significant share of FPCA runtime (else recorded as "not warranted, with evidence")
...
- [x] **GAP-04**: fdars capabilities that exceed scikit-fda are documented (e.g. model explainability, SPM/control charts, seasonal decomposition, streaming depth) so the audit reflects strengths, not only gaps

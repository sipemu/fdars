...
- [ ] **NAME-01**: `geometric_median` is consolidated from its lone `_1d`/`_2d` suffix forms onto a single `Dim`-dispatched signature routing to byte-identical private `_impl` bodies. (AUD-19)
- [ ] **NAME-02**: The `hausdorff_*` family is consolidated onto a `Dim`-dispatched signature (byte-identical `_impl` bodies). (AUD-20)
- [ ] **NAME-03**: `functional_spatial_*` and `kernel_functional_spatial_*` are consolidated onto `Dim` dispatch (byte-identical `_impl` bodies). (AUD-21)
...
- [ ] **NAME-05**: `LpeerResult` is renamed to `LocalPeerResult` (matching the `PeerResult` sibling), with all references, re-exports (`lib.rs`, `prelude.rs`), examples, and doctests updated. (AUD-23)

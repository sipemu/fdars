...
- [x] **PER-04**: User can fit longitudinal PEER via a public `lpeer(...)` estimator that extends PEER to repeated per-subject measurements with subject-level random effects, fitted through `famm::fit_scalar_mixed_model` (REML EM), returning a result struct with the (time-varying) coefficient function and variance components.
- [x] **PER-05**: User can predict on new curves out-of-sample from a fitted PEER/lpeer result (`predict`), access the coefficient function and fitted values, and reach the API from the crate root + prelude; a module doctest demonstrates the end-to-end workflow.
...
| PER-04 | Phase 68 | Complete |
| PER-05 | Phase 68 | Complete |

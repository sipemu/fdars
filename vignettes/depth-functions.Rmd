...
```{r depth-rpd}
depths_rpd <- depth.RPD(fd, nproj = 50, nderiv = 1)
head(depths_rpd)
```
...
med_fm <- func.med.FM(fd)
med_mode <- func.med.mode(fd)
med_rp <- func.med.RP(fd, nproj = 50)
...
```{r trimmed-mean}
# 10% trimmed mean
trim_fm <- func.trim.FM(fd, trim = 0.1)
trim_mode <- func.trim.mode(fd, trim = 0.1)
...
trimvar_fd <- func.trimvar.FM(fd, trim = 0.1)
```

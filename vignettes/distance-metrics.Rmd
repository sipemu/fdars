...
Use `metric.dist()` for a unified interface:

```{r metric-dist}
# Different methods via single function
d1 <- metric.dist(fd, method = "lp", p = 2)
d2 <- metric.dist(fd, method = "DTW")
d3 <- metric.dist(fd, method = "hausdorff")
```
...
km_l1 <- kmeans.fd(fd_large, ncl = 3, metric = "L1", seed = 42)

# K-means with custom metric function
km_dtw <- kmeans.fd(fd_large, ncl = 3, metric = metric.DTW, seed = 42)

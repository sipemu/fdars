...
cov_gauss <- cov.Gaussian(variance = 1, length_scale = 0.2)
print(cov_gauss)
...
for (ls in c(0.05, 0.2, 0.5)) {
  fd <- make_gaussian_process(n = 5, t = t,
                              cov = cov.Gaussian(length_scale = ls),
                              seed = 42)
...
```{r cov-exponential}
cov_exp <- cov.Exponential(variance = 1, length_scale = 0.2)
...
for (nu in c(0.5, 1.5, 2.5, Inf)) {
  fd <- make_gaussian_process(n = 5, t = t,
                              cov = cov.Matern(length_scale = 0.2, nu = nu),
                              seed = 42)
...
```{r cov-brownian}
cov_brown <- cov.Brownian(variance = 1)
...
cov_per <- cov.Periodic(variance = 1, length_scale = 0.5, period = 1)
...
```{r cov-linear}
cov_lin <- cov.Linear(variance = 1, offset = 0)
...
```{r cov-polynomial}
cov_poly <- cov.Polynomial(variance = 1, offset = 1, degree = 3)
...
```{r cov-white}
cov_white <- cov.WhiteNoise(variance = 0.5)
...
### Multiplication (cov.mult)

Product of covariance functions:

```{r cov-mult}
# Locally periodic: smooth envelope modulating periodic behavior
cov_envelope <- cov.Gaussian(variance = 1, length_scale = 0.5)
cov_periodic <- cov.Periodic(period = 0.2)
cov_local_per <- cov.mult(cov_envelope, cov_periodic)
...
Note: `cov.Brownian` and `cov.Periodic` only support 1D domains.
...
```{r reproducibility}
fd1 <- make_gaussian_process(n = 3, t = t, cov = cov.Gaussian(), seed = 123)
fd2 <- make_gaussian_process(n = 3, t = t, cov = cov.Gaussian(), seed = 123)
...
fd_gauss <- make_gaussian_process(n = 3, t = t, cov = cov.Gaussian(), seed = 1)
plot(fd_gauss, main = "Gaussian (very smooth)")

fd_mat25 <- make_gaussian_process(n = 3, t = t, cov = cov.Matern(nu = 2.5), seed = 1)
plot(fd_mat25, main = "Matern 5/2")

fd_mat15 <- make_gaussian_process(n = 3, t = t, cov = cov.Matern(nu = 1.5), seed = 1)
plot(fd_mat15, main = "Matern 3/2")

fd_exp <- make_gaussian_process(n = 3, t = t, cov = cov.Exponential(), seed = 1)
